...
const currencySymbols: Record<DisplayCurrency, string> = { USD: "$", EUR: "€", GBP: "£", CAD: "$", AUD: "$" };

export function CurrencySwitcher({ mobile = false }: { mobile?: boolean }) {
  const { currency, setCurrency } = useCurrency();
  return <div className={mobile ? "rounded-md bg-surface p-3 shadow-card" : "w-11"}>
    {mobile ? <p className="mb-2 px-1 text-sm font-semibold text-text-dim">Display currency</p> : null}
    <StyledSelect value={currency} options={options} onValueChange={(value) => setCurrency(value as DisplayCurrency)} ariaLabel="Display currency" className={mobile ? "currency-switcher" : "currency-switcher currency-switcher--compact"} menuClassName="min-w-[104px]" triggerLabel={mobile ? undefined : currencySymbols[currency]} />
