...
export function SeDollPdpFreebieBlock({ product }: { product: Pick<Product, "handle" | "extended"> }) {
  ...
  return (
    <section className="mt-6 overflow-hidden rounded-[16px] border border-gold-500/25 bg-ink-900/80" aria-labelledby="se-pdp-bonuses-heading">
      <div className="border-b border-gold-500/16 bg-gold-500/[0.08] px-5 py-4">
        <p className="flex items-center gap-2 text-sm font-semibold text-gold-300">
        ...
        <p className="mt-1 text-sm leading-6 text-ivory-300">For custom orders placed 1–30 September 2026.</p>
      </div>
  ...
  return (
    <article className="overflow-hidden rounded-[20px] border border-gold-500/18 bg-ink-900/72 shadow-panel">
      <div className="grid lg:grid-cols-[minmax(320px,0.78fr)_minmax(0,1.22fr)]">
        <div className="relative aspect-[4/3] min-h-[300px] overflow-hidden bg-ink-950 lg:aspect-auto">
          <Image src={promo.cardImage} alt={promo.cardAlt} fill priority sizes="(min-width: 1024px) 38vw, 100vw" className="object-cover" />
        </div>
