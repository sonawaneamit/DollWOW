...
export async function POST(request: Request) {
  ...
  if (!parsed.success || !isVisualizerProduct(parsed.data.productHandle)) {
  ...
  if (!product) return NextResponse.json({ error: "This doll is not currently available." }, { status: 404 });
  ...
  const visualSelections = resolveVisualizerSelections(config, parsed.data.selections);
  ...
  return NextResponse.json({
    item: {
      ...
      attributes: [
        ...
        { key: "DollWow Visualizer", value: "Appearance choices added from Doll Visualizer™" }
      ],
