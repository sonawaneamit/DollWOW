...
export const metadata: Metadata = {
  ...
  description: "See certifications and written confirmations brands have issued authorizing DollWow to sell their products."
};
    ...
    <div className="shop-visual-shell min-h-screen">
      <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="max-w-3xl">
          <p className="text-sm font-semibold  text-gold-300">Certifications</p>
          <h1 className="mt-3 text-4xl font-semibold tracking-tight text-ivory-50 sm:text-5xl">Brand certifications</h1>
          <p className="mt-4 text-base leading-7 text-ivory-300">
            These documents and written confirmations are issued by brands to authorize DollWow to sell their products.
          </p>
        </div>
        ...
        <section className="mt-10 rounded-[8px] border border-gold-500/18 bg-ink-900/62 p-6 sm:p-7" aria-labelledby="live-authorized-brands">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h2 id="live-authorized-brands" className="mt-2 text-2xl font-semibold text-ivory-50">Brands represented by DollWow</h2>
            </div>
            ...
            {certificateEntries.map((entry) => (
              <article key={entry.id} id={entry.id} className="overflow-hidden rounded-[8px] border border-gold-500/18 bg-ink-900/70">
                <a href={entry.certificateSrc!} target="_blank" rel="noreferrer" className="group relative block aspect-[4/3] bg-[#f8efe8]">
                  <Image src={entry.certificatePreviewSrc!} alt={`${entry.brand} certification authorizing DollWow as a retailer`} fill sizes="(max-width: 640px) 100vw, (max-width: 1280px) 50vw, 33vw" className="object-contain p-4 transition duration-300 group-hover:scale-[1.02]" />
                </a>
                <div className="border-t border-gold-500/12 p-5">
                  <p className="flex items-center gap-2 text-sm font-semibold  text-gold-300"><BadgeCheck className="h-4 w-4" /> Brand-issued certification</p>
        ...
        <section className="mt-12 rounded-lg border border-border bg-surface p-6 shadow-card sm:p-7" aria-labelledby="written-confirmations-heading">
          ...
          <h2 id="written-confirmations-heading" className="mt-2 text-2xl font-semibold text-ivory-50">Additional brand confirmations</h2>
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            {writtenConfirmations.map((entry) => (
              <article key={entry.id} className="rounded-[8px] border border-gold-500/16 bg-ink-950/50 p-5">
                ...
                <p className="mt-2 text-sm leading-6 text-ivory-400">The brand has confirmed in writing that DollWow is approved to sell its products.</p>
