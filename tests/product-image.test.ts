...
describe("protected product images", () => {
  it("deduplicates sources and assigns stable positions", () => {
    ...
    expect(protectedProductImageUrlFor(product, product.images[1])).toBe("/product-media/sample-doll/1?v=3");
    expect(protectedProductImageUrlFor(product, product.images[1], "thumb")).toBe("/product-media/sample-doll/1?v=3&size=thumb");
  });
  ...
  it("removes source URLs from the public product payload", () => {
    ...
    expect(protectedProduct.featuredImage?.url).toBe("/product-media/sample-doll/0?v=3");
    expect(protectedProduct.images[1].url).toBe("/product-media/sample-doll/1?v=3");
  });
});
