...
export async function sendVisualizerLookEmail(input: {
  ...
  return sendEmail({
    ...
    html: `<div style="font-family:Arial,sans-serif;color:#291f1b;max-width:620px;margin:auto"><span style="display:none;max-height:0;overflow:hidden">See the doll with the appearance choices you selected.</span><p style="font-size:12px;letter-spacing:.12em;color:#b5471f">DOLL VISUALIZER™</p><h1 style="font-size:28px">Your preview is ready</h1><p>We created a Doll Visualizer™ preview using the product photo and appearance choices you selected. Open the attachments to compare the preview with the original doll and continue exploring your options.</p><p style="line-height:1.6"><strong>${escapeHtml(input.productName)}</strong><br>${escapeHtml(optionText)}</p><p><a href="${input.productUrl}" style="display:inline-block;padding:14px 20px;border-radius:12px;background:#b5471f;color:#fff;text-decoration:none;font-weight:700">Return to the doll</a></p><p style="font-size:12px;line-height:1.5;color:#6e615a">${disclaimer}</p><p style="font-size:12px;line-height:1.5;color:#6e615a">Your identity and account details are never displayed with a preview.</p><p>Have a question about an option? Reply to this email or ask the DollWOW team.</p></div>`,
    attachments: [
