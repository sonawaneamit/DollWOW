...
export async function sendDollVueLookEmail(input: {
  to: string;
  productName: string;
  productUrl: string;
  ...
  const previewAttachment = await dataUrlAttachment(input.previewDataUrl, "dollwow-dollvue-preview.webp");
  return sendEmail({
    to: input.to,
    bcc: input.to.toLowerCase() === "hello@dollwow.com" ? undefined : "hello@dollwow.com",
    replyTo: "hello@dollwow.com",
    subject: "Your DollVue™ preview is ready",
    text: `Your preview is ready.\n\nWe created a DollVue™ preview using the product photo and appearance choices you selected.\n\n${input.productName}\n${optionText}\n\nReturn to the doll: ${input.productUrl}\n\n${disclaimer}\n\nHave a question about an option? Reply to this email or ask the DollWOW team.`,
    html: `<div style="font-family:Arial,sans-serif;color:#291f1b;max-width:620px;margin:auto"><span style="display:none;max-height:0;overflow:hidden">See the doll with the appearance choices you selected.</span><p style="font-size:12px;letter-spacing:.12em;color:#b5471f">DOLLVUE™</p><h1 style="font-size:28px">Your preview is ready</h1><p>We created a DollVue™ preview using the product photo and appearance choices you selected. Compare the original and your preview below, then return to the doll when you are ready.</p><div style="display:flex;gap:12px;margin:20px 0"><div style="width:50%"><p style="font-size:11px;font-weight:700;letter-spacing:.08em">ORIGINAL</p><img src="cid:dollwow-original" alt="Original product photo" style="display:block;width:100%;height:auto;border-radius:12px"></div><div style="width:50%"><p style="font-size:11px;font-weight:700;letter-spacing:.08em;color:#b5471f">YOUR PREVIEW</p><img src="cid:dollwow-preview" alt="DollVue preview" style="display:block;width:100%;height:auto;border-radius:12px"></div></div><p style="line-height:1.6"><strong>${escapeHtml(input.productName)}</strong><br>${escapeHtml(optionText)}</p><p><a href="${input.productUrl}" style="display:inline-block;padding:14px 20px;border-radius:12px;background:#b5471f;color:#fff;text-decoration:none;font-weight:700">Return to the doll</a></p><p style="font-size:12px;line-height:1.5;color:#6e615a">${disclaimer}</p><p style="font-size:12px;line-height:1.5;color:#6e615a">Your identity and account details are never displayed with a preview.</p><p>Have a question about an option? Reply to this email or ask the DollWOW team.</p></div>`,
    attachments: [
